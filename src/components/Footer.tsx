import "./Footer.css";

export default function Footer() {
  return (
    <footer className="footer">
      <div className="footer__copyright">
        © {new Date().getFullYear()}{" "}
        <a
          href="https://www.aapospotosi.com"
          target="_blank"
          rel="noreferrer"
        >
          aapospotosi.sitio.serviciostigobusiness.com
        </a>
      </div>

      <h2 className="footer__title">AAPOS POTOSI</h2>

      <p className="footer__line">
        Horarios de atención: Lunes a viernes de 8:00 a 12:00 y de 14:00 a
        18:00
      </p>
      <p className="footer__line">Correo electrónico: aapos@aapos.com.bo</p>

      <a
        className="footer__social"
        href="https://www.facebook.com/AAPOSOFICIAL"
        target="_blank"
        rel="noreferrer"
      >
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path d="M22 12.06C22 6.5 17.52 2 12 2S2 6.5 2 12.06c0 5 3.66 9.15 8.44 9.94v-7.03H7.9v-2.9h2.54V9.85c0-2.5 1.49-3.89 3.78-3.89 1.1 0 2.24.2 2.24.2v2.46h-1.26c-1.24 0-1.63.77-1.63 1.56v1.88h2.78l-.44 2.9h-2.34V22c4.78-.79 8.44-4.94 8.44-9.94Z" />
        </svg>
        <span>AAPOSOFICIAL</span>
      </a>
    </footer>
  );
}
